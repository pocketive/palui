...
void send_api_request(const String& prompt)
{
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        responseText = "Servers busy, try again later";
    } else if (httpCode == -11) {
        responseText = "Connection timeout. Check WiFi.";
    } else if (httpCode < 0) {
        responseText = "Network error";
    } else {
        responseText = "HTTP " + String(httpCode);
    }
...
static bool connect_to_saved_wifi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (WiFi.status() == WL_CONNECT_FAILED) error = "Wrong password";
        else if (WiFi.status() == WL_NO_SSID_AVAIL) error = "Network not found";
...
static void send_message(lv_event_t * e) {
    ...
    if (!wifi_connected) {
        if (!connect_to_saved_wifi()) return;
    }
...
void main_gui(void) {
    ...
    if (!first_message_sent) {
        ...
        const char* welcome_messages[] = {
            "ready to help.", "how may I help you?", "what can I do for you?",
            "here to assist you.", "let's get started.", "how can I assist?",
            "here to help.", "welcome back.", "ready when you are.", "at your service."
        };
        int random_index = random(0, 10);
...
void show_splash_screen() {
    ...
    lv_label_set_text(palui_label, "PalUI Any");
